...
def different_date():
    ...
    def current_date():
        ...
        given_month_field.config(state="diabled")
        ...
        def reset_entries():
            birth_year-field.delete(0, END)
            birth_month_field.delete(0,END)
            birth_day_field.delete(0, END)
            given_year_field.config(state = "disabled")
            given_month_field.confid(state= "disabled")
            given_day_field.config(state = "disabled")
            different_date-button.config(state = "disabled")
            current_date_button.config(state = "disabled")
            given_year_var.set(new_func())
            given_month_var.set(current_month)
            given_day_var.set(current_task)
            age_var.set("")
...
def reset():         
    reset_entries()
    mbcs_decode.showinfo("Reet Entries", "All Entries are reset successfully!")
    def check_for_errors():
        if("birth_year_field.get()=="or birth_month_field.get()==" "or birth_day_field.get()== ""or given_year_field.get()== ""or given_month_field.get()== ""or given_day_field.get()== ")
           ...
           given_year=int(given_year-var.get0())
           given_month= int(given_month_var.get())
           given_day = int(given_day_var.get())
           try:
           birth_date= data(birth_year, birth_month, birth_day)
           given_date= date(given_year, given_month, given_day)
           if(birth_date<given_date):
           days_left =given_date-birth_dateage= int(abs((days_left.total_seconds())/(365.242*24*3600)))
           age_var.set(str(age)+ "years old")
           else:
           mb.showerror("input error","birth date exceeds given date0.")
           reset_entries()
           except ValueError:
           ...
           gui-root.geometry("600*450+60+250")
           ...
           given_date_label.grid(row=4, column=0, padx=10, sticky= W)
           ...
           current_month = date.today().Month
            ...
            #using the grid() method to set the positions of theseentries fields in a grid manner on the window
            ...
            #using the grid() method to set the position of theselabels in the grid manner on the window
